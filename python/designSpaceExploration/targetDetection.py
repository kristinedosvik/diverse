...
def OC_ACE_R(frames, framesamples, bands):
    return correlation_matrix(frames, framesamples, bands) + quadratic_matrix_invertion(bands) + matrix_vector_multiplication(bands, bands) + dot_product(bands) + frames * framesamples * ( matrix_vector_multiplication(bands, bands) * 2 *  dot_product(bands, bands) + 2 * multiplication() + division())
