...
def DOS_spectral_binning(frames, framesamples, bands, binningfactor):
    new_frames = frame
    new_framesamples = framesamples
    new_bands = np.ceil(bands/binningfactor)
    return new_frames, new_framesamples, new_bands
...
def OC_spatial_binning(frames, framesamples, bands, binningfactor, whatToBin):
    if(whatToBin == "frames"):
        return framesamples * (np.floor(frames/binningfactor) * binningfactor * bands * additions() + (frames % binningfactor) * bands * additions())
    else:
        return fames * (np.floor(framesamples/binningfactor) * binningfactor * bands * additions() + (framesamples % binningfactor) * bands * additions())
